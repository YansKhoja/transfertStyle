...
def get_feature_representations(model, content_image, style_image, nbr_layers):
    ...
    style_features = [style_layer[0] for style_layer in style_outputs[:nbr_layers]]
...
def get_outputs(style_layers, content_layers, custom_model, style_image, content_image, num_layers):
...
def style_content_loss(outputs,target,loss_weights, num_layers):
    ...
    style_targets = dict(target['style'].items())
    content_targets = dict(target['content'].items())

    style_weight, content_weight = loss_weights 
    num_style_layers, num_content_layers = num_layers

    print('style_outputs : '+str(len(style_outputs)))
    print(style_outputs)
    ...
    for key, valeur in style_outputs.items():
        ...
        print(valeur)
    print('')
    # style_outputs = np.array(style_outputs)
    # style_targets = np.array(style_targets)
    
    # style_outputs = [tf.convert_to_tensor(style_output) for style_output in style_outputs] 
    # content_outputs = [tf.convert_to_tensor(content_output) for content_output in content_outputs]
    # style_targets = [tf.convert_to_tensor(style_target) for style_target in style_targets] 
    # content_targets = [tf.convert_to_tensor(content_target) for content_target in content_targets]
    
    # print(type(style_outputs[1]))
    # style_outputs = tf.constant(style_outputs[1])
    # print(style_outputs)
    
    # print('style_outputs : '+str(len(style_outputs)))
    # print('')
    # for indice in range(1,len(style_outputs)):
        
    #     print('style_outputs ' + str(indice) +' : ' + str(style_outputs[indice]))
    #     print('')
        # print('content_outputs : '+str(len(content_outputs)))
        # print('')
        # print('style_targets : '+str(len(style_targets)))
        # print('')
        # print('content_targets : '+str(len(content_targets)))
        # print('')
    # tf.reduce_mean((style_outputs-style_targets)**2)
    
    style_loss = tf.add_n([tf.reduce_mean((content_outputs[name]-content_targets[name])**2) for name in content_outputs.keys()])
    print('style_loss : '+str(style_loss.shape))
    print('')
    style_loss *= style_weight / num_style_layers

    content_loss = tf.add_n([tf.reduce_mean((content_outputs[name]-content_targets[name])**2) for name in content_outputs.keys()])
    content_loss *= content_weight / num_content_layers
    print('content_loss : '+str(content_loss.shape))
    print('')
...
@tf.function()
def train_step(opt, image, style_layers,content_layers, custom_model, style_image, content_image, num_layers,target,loss_weights):
   print('=========== train_step =============')
   print("opt : " + str(opt))
   ...
   print("style_image : "+str(style_image))
   print("content_image : " + str(content_image))
   print("num_layers : "+str(num_layers))
   print("target" + str(target))
   print("loss_weights" + str(loss_weights))
   with tf.GradientTape() as tape:
    outputs =  get_outputs(style_layers,content_layers, custom_model, style_image, content_image, num_layers)
